...
const Cluster = ({ team }) => {
    ...
    const clusterPositions = useGameStore((state) => state.clusterPositions);
    const updateClusterPositions = useGameStore(
        (state) => state.updateClusterPositions
    );
    ...
    }, [team, unguessedWords.length, updateClusterPositions]);
            ...
            class {
                ...
                markWordCollision(position) {
                    ...
                    const matchingDir = allDirs.find(
                        (dir) =>
                            Math.abs(dx - dir.x) < 0.1 &&
                            Math.abs(dy - dir.y) < 0.1
                    );
    ...
    const adjustPosition = useCallback(
        (position, index, allPositions) => {
            ...
            const clampedAbsolutePosition =
                clampAbsolutePosition(absolutePosition);
            ...
            while (attempts < MAX_ATTEMPTS) {
                if (queue.isEmpty()) {
                    if (attempts < 8) {
                        ...
                        attempts < 50 &&
                        queue.wordCollisionDirs.size > 0
                    ) {
                        ...
                        return {
                            x:
                                clampedRandomPosition.x -
                                initialPositions[index].x,
                            y:
                                clampedRandomPosition.y -
                ...
                const clampedNextOffset = {
                    x:
                        clampedNextAbsolutePosition.x -
                        initialPositions[index].x,
                    y:
                        clampedNextAbsolutePosition.y -
                        initialPositions[index].y,
                ...
                if (
                    ...
                    const currentRect = getRectForWord(
                        index,
                        clampedNextOffset
                    );
        ...
        [wordDimensions, initialPositions, constraintsRef]
    );

    const getRectForWord = useCallback(
        (index, position) => {
            const dimensions = wordDimensions[index] || { width: 0, height: 0 };
            ...
            return {
                ...
                bottom:
                    initialPositions[index].y + offset.y + dimensions.height,
            };
        },
        [wordDimensions, initialPositions]
    );

    const isPositionValid = useCallback(
        (position, index, allPositions, containerBounds) => {
